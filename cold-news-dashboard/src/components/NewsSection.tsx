...
export function NewsSection({ title, items, color }: SectionProps) {
    ...
    return (
        <div className="mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className={`flex items-center gap-3 mb-6 pb-2 border-b ${borderColor}`}>
                <div className={`w-1 h-6 rounded-full ${color === 'cyan' ? 'bg-cyan-500 shadow-[0_0_10px_#06b6d4]' :
                    color === 'blue' ? 'bg-blue-500 shadow-[0_0_10px_#3b82f6]' :
                        'bg-purple-500 shadow-[0_0_10px_#a855f7]'}`} />
                <h2 className={`text-xl font-bold tracking-wider uppercase ${titleColor}`}>
                    {title} <span className="text-slate-600 text-sm ml-2 font-mono">({items.length})</span>
                </h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                {items.length === 0 && (
                    <div className="col-span-full py-12 text-center text-slate-600 font-mono text-sm border border-dashed border-slate-800 rounded-lg">
